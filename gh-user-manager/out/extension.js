...
function activate(context) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('ghUserManager.addProfile', async () => {
        await collectProfileInfo();
    }));
    // Other commands remain simple
    context.subscriptions.push(vscode.commands.registerCommand('ghUserManager.switchProfile', async () => {
        const config = (0, profile_1.listProfiles)();
        ...
        if (selected) {
            await (0, profile_1.switchProfile)(selected.label);
    ...
    // 	})
    //   );
    context.subscriptions.push(vscode.commands.registerCommand('ghUserManager.listProfiles', async () => {
        const config = (0, profile_1.listProfiles)();
        const profiles = config.profiles;
        const current = config.current;
        if (Object.keys(profiles).length === 0) {
        ...
        if (selected) {
            ...
            vscode.window.showInformationMessage(`📄 Profile: ${profileName}\n` +
    ...
    context.subscriptions.push(vscode.commands.registerCommand('ghUserManager.deleteProfile', async () => {
        const config = (0, profile_1.listProfiles)();
        const items = Object.keys(config.profiles).map(name => ({
            label: name,
            description: config.profiles[name].email
...
async function collectProfileInfo() {
    ...
    await (0, profile_1.addProfile)({ name, email, username, auth, token: token || null });
    vscode.window.showInformationMessage(`Profile "${name}" added.`);
}
function deactivate() { }
